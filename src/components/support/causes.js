    ...
    return(
        <section>
            {causesDetails.map((cause)=>(
                <div key={cause.id} className= {`w-full flex ${cause.id % 2 ===1? "flex-row":"flex-row-reverse" } ${cause.id===1? "bg-[#004265]":cause.id===2?"bg-[#3EBFF980]":cause.id===3? "bg-[#622676]":cause.id===4? "bg-[#FF9DF080]": cause.id===5? "bg-[#FCCE37]": cause.id===6?"bg-[#FFD44699]":""} max-md:flex-col`}  >
                   <div className=" p-40 max-2xl:p-18 w-3/5 max-lg:p-8 max-md:w-full ">
                        <h1 className={`font-[700]  text-[70px] max-2xl:text-[50px]  max-lg:text-[40px] max-sm:text-2xl mb-10  max-2xl:mb-4  ${cause.id===1 || cause.id===3? 'text-white':cause.id===2 || cause.id===5?'text-[#004265]': cause.id===4? 'text-[#622676D1]':'text-black' } `}>{cause.title}</h1>
                        <p className={`font-[500]  text-[40px] max-2xl:text-[30px]  max-lg:text-[20px] max-sm:text-[16px]  pl-15 max-lg:pl-8 max-sm:p-4 ${cause.id===1 || cause.id===3? "text-white" : "text-black"} `}>{cause.details}</p>
                   </div>

                   
                    <div className="w-2/5 relative max-md:w-full max-md:h-[497px]  ">
                        <Image src="/images/supportpage/children-img.png" alt="children from Dalit communities"
                          fill
